...
import { REPOSITORY_TAG_FILTER_OPTIONS, REPOSITORY_USER_ACCESS_FILTER_OPTIONS } from '../../config/table_filter';
...
const RepositoryAccessViewPage = () => {

  const [userAccesList, setUserAccessList] = useState<RepositoryUserAccessInfo[]>(mockRepositoryUserAccess)
  ...
  return (<div className="flex flex-column p-2 pt-4 gap-3 ">
    <div className="bg-white border-round-lg flex flex-column gap-2">
      <div className="flex flex-column pt-2 gap-2">
        <div className="flex flex-row justify-content-between pt-2 pb-2">
          <div className="pl-3">
            ...
            <span className=' font-medium text-lg'>
              <span className='cursor-pointer underline text-blue-600'>team-platform-eng</span> <span className='text-sm'>/</span>order-backend
            </span>
            &nbsp;&nbsp;
        ...
        <div className='flex flex-row pl-3 pr-3 pb-2'>
          <span className='font-semibold'>Access Inheritance & Rules&nbsp;&nbsp;
            <span className='cursor-pointer pi pi-info-circle text-xs'></span></span>
        </div>
        <div className='flex flex-column w-full gap-3 pl-4 pr-4'>
          ...
          <div className='flex w-full justify-content-between align-items-center  flex-row flex-grow-1 gap-5'>
            ...
            <div className='flex flex-column gap-2 max-w-28rem w-9  border-red-500 border-1 border-round-xl p-2 pl-3'>
              ...
              <div className='text-sm'>
                Automatically has full access to all repositories. No repository-level assignment needed or allowed.                </div>
            </div>
          </div>
          ...
          <div className='flex w-full justify-content-between align-items-center flex-row flex-grow-1 gap-5'>
            <div className='flex flex-column gap-2 max-w-28rem w-5  border-primary-400 border-1 border-round-xl p-2 pl-3'>
              ...
              <div className='text-sm'>
                Can read, write, and manage repositories.
              </div>
            </div>
            <div className='flex justify-content-center align-items-center text-primary-400'>
              <span className='pi pi-chevron-right'></span>
            ...
            <div className='flex flex-column gap-2 max-w-28rem w-9  border-red-500 border-1 border-round-xl p-2 pl-3'>
              ...
              <div className='text-sm'>
                Automatically has developer access to all repositories. Cannot be changed at repository level.
              </div>
            </div>
          </div>
          ...
          <div className='flex w-full justify-content-between align-items-center flex-row flex-grow-1 gap-5'>
            ...
            <div className='flex flex-column gap-2 max-w-28rem w-9  border-primary-400 border-1 border-round-xl p-2 pl-3'>
              ...
              <div className='text-sm flex flex-column'>
                <div>Can be granted <span className='font-semibold'>Developer</span> access to specific repositories.</div>
              </div>
            </div>
          </div>
          ...
          <div className='flex  surface-50 border-left-3 gap-2 m-1 p-2 border-round-lg'>
            ...
            <span className='text-sm flex-grow-1'>Repository level only supports Developer and Guest access levels. Maintainer access is not available at the repository level. Additionally, user access level at namespace cannot be downgraded at repository level (e.g., Developer at namespace cannot become Guest at repository).</span>
          </div>
        </div>
        <TabView>
          <TabPanel
          ...
          >
            <div className="flex flex-row align-items-center justify-content-between pl-3 pr-2 ">
              <div className='flex align-items-center  flex-grow-1'>
                ...
                <ChipsFilter filterOptions={REPOSITORY_USER_ACCESS_FILTER_OPTIONS} handleFilterChange={handleUserAccessFilter} />
              </div>
              <Button
                size="small"
                outlined
            ...
            >
              ...
              <Column field='access_level' header="Access Level" body={(data: RepositoryUserAccessInfo) => (<div>
                <span>{data.access_level}</span>
                {data.is_inherited && <span>    &nbsp;
                  <Chip className='pl-2 pr-2 p-0  bg-white border-400 border-1' style={{ fontSize: '0.60rem' }} label='inherited' /></span>}
          ...
          >
            <div className="flex flex-row align-items-center justify-content-between pl-3 pr-2 ">
              <div className='flex align-items-center  flex-grow-1'>
                <div className=" text-sm"> Select Filter</div>
                <ChipsFilter filterOptions={REPOSITORY_TAG_FILTER_OPTIONS}
            ...
            <DataTable value={tagsList}
              paginator
              paginatorLeft={<div className="text-xs">82 tags found</div>}
              rows={8}
              totalRecords={mockRepositoryTags.length}
            >
              <Column field='tag' header="Tag" body={(data: RepositoryTagInfo) => (<div><span>{data.tag}</span>  &nbsp;
                {data.stable && <Chip className='pl-2 pr-2 p-0  bg-white border-400 border-1' style={{ fontSize: '0.60rem' }} label='stable' />}</div>)} sortable />
              <Column header="Platform" body={(data: RepositoryTagInfo) => (<div>
                <span>{data.platform_arch}</span>  &nbsp;/ <span>{data.platform_os}</span>
              </div>)} />

              <Column header="Pushed At" field='last_pushed' body={(data: RepositoryTagInfo) => data.last_pushed.toUTCString()} sortable />
              <Column
                align='right'
                body={(data) => {
                  return (<div className='flex justify-content-end'>
...
export default RepositoryAccessViewPage;
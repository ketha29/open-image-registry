...
CREATE TABLE IF NOT EXISTS UPSTREAM_OCI_REGISTRY_STORAGE_CONFIG(
  REGISTRY_ID TEXT NOT NULL,
  STORAGE_LIMIT REAL DEFAULT 100 CHECK(STORAGE_LIMIT >= 100), -- 10GB default, min 100MB
...
CREATE TABLE IF NOT EXISTS IMAGE_TAG_MANIFEST_MAPPING(
  TAG_ID TEXT NOT NULL,
  MANIFEST_DIGEST TEXT NOT NULL,
  CREATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  UPDATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  UNIQUE(TAG_ID, MANIFEST_DIGEST),
  FOREIGN KEY (TAG_ID) REFERENCES IMAGE_TAG(ID) ON DELETE CASCADE
);

CREATE TRIGGER IF NOT EXISTS tr_image_tag_manifest_mapping_updated_at
  AFTER UPDATE ON IMAGE_TAG_MANIFEST_MAPPING
BEGIN
  UPDATE IMAGE_TAG_MANIFEST_MAPPING SET UPDATED_AT = CURRENT_TIMESTAMP 
  WHERE TAG_ID = NEW.TAG_ID AND MANIFEST_DIGEST = NEW.MANIFEST_DIGEST;
END;
...
CREATE TABLE IF NOT EXISTS IMAGE_MANIFEST (
  ...
  CREATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  UPDATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  UNIQUE (REGISTRY_ID, NAMESPACE_ID,REPOSITORY_ID, DIGEST),
  FOREIGN KEY (REPOSITORY_ID) REFERENCES REGISTRY_REPOSITORY(ID) ON DELETE CASCADE,
  FOREIGN KEY (REGISTRY_ID) REFERENCES UPSTREAM_OCI_REGISTRY(ID) ON DELETE CASCADE,
  FOREIGN KEY (NAMESPACE_ID) REFERENCES REGISTRY_NAMESPACE(ID) ON DELETE CASCADE
);

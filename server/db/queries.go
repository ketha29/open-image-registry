...
const GetImageManifestByDiegest = `SELECT MANIFEST_CONTENT, MEDIA_TYPE, SIZE FROM IMAGE_MANIFEST im JOIN REGISTRY_REPOSITORY rr ON im.REPOSITORY_ID = rr.ID JOIN REGISTRY_NAMESPACE rn ON im.NAMESPACE_ID = rn.ID WHERE im.REGISTRY_ID = ? AND rn.NAMESPACE = ? AND rr.NAME = ?`
